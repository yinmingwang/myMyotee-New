...
function dataParse(d, b, c, a, e) {
    if (c.length > 0) { $("." + b).remove();
        d.selectAll("." + b).data(c).enter().append("g").attr("class", function(h) {
            var f = d3.select(this);
            var g = f.append("path").attr("d", h.path);
            h.style.forEach(function(i) { g.attr(i.attr, i.val) });
            return b });
        d.attr("transform", "translate(" + a + "," + e + ")") } };
function bubbleDataParse(c, a, b) {
    $("." + a).remove();
    if (b.path.length > 0) { c.selectAll("." + a).data(b.path).enter().append("g").attr("class", function(g) {
            var e = d3.select(this);
            var f = e.append(g.type);
            g.style.forEach(function(d) {
                if (d.attr == "text") { f.text(d.val) } else { f.attr(d.attr, d.val) } });
            return a });
        if ((b.cX != 0) || (b.cY != 0)) { c.attr("transform", "translate(" + b.cX + "," + b.cY + ")") } } }
